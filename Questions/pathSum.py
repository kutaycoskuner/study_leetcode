...
def solution(root, targetSum):  
    ...
    if root == None:
        return False
    # == base case
    # :: leaf node a ulastiysak tarsum sifir mi diye bak
    # :: evetse return yoksa devam
    if root.left == None and root.right == None: 
        return targetSum == root.val
    # :: go depth first search recursive
    hasTargetSumLeft = solution(root.left, targetSum-root.val)
    hasTargetSumRight = solution(root.right, targetSum-root.val)
    # :: tree bittiyse false
    return hasTargetSumLeft or hasTargetSumRight
